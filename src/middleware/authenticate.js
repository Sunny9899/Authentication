...
module.exports = async(req,res,next)=>{
    ...
    if(! bearerToken || ! bearerToken.startsWith('Bearer ')) return res.status(400).json({status:"failed", message:"Please provide valid Token"});
    ...
    //If no user found we will throw error
    if(! user) return res.status(400).json({status:"failed", message:"Please provide valid Token"});
